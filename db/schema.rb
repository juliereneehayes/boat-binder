...
ActiveRecord::Schema[8.1].define(version: 2026_08_11_193000) do
  ...
  create_table "billing_checkout_attempts", force: :cascade do |t|
    ...
    t.index ["account_id"], name: "idx_billing_checkout_attempts_one_active_per_account", unique: true, where: "((status)::text = ANY ((ARRAY['creating'::character varying, 'open'::character varying, 'submitted'::character varying])::text[]))"
    ...
    t.check_constraint "status::text = ANY (ARRAY['creating'::character varying, 'open'::character varying, 'submitted'::character varying, 'completed'::character varying, 'canceled'::character varying, 'expired'::character varying, 'replaced'::character varying]::text[])", name: "chk_billing_checkout_attempts_status"
  end
